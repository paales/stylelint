...
function augmentConfigBasic(stylelint, config, configDir, allowOverrides) {
	return Promise.resolve()
		...
		.then((augmentedConfig) => extendConfig(stylelint, augmentedConfig, configDir))
		.then((augmentedConfig) => absolutizePaths(augmentedConfig, configDir));
}
...
function augmentConfigFull(stylelint, cosmiconfigResult) {
	...
	return augmentConfigBasic(stylelint, config, configDir, true)
		.then((augmentedConfig) => addPluginFunctions(augmentedConfig))
		.then((augmentedConfig) => addProcessorFunctions(augmentedConfig))
...
function absolutizePaths(config, configDir) {
	...
	if (config.plugins) {
		config.plugins = /** @type {string[]} */ ([])
			.concat(config.plugins)
			.map((lookup) => getModulePath(configDir, lookup));
	}
...
function extendConfig(stylelint, config, configDir) {
	...
	return loadExtends.then((resultConfig) => mergeConfigs(resultConfig, originalWithoutExtends));
}
